...
namespace phystest
{
    public class CharacterController : Updateable, IEndOfTimeStepUpdateable
    {
        ...
        /// <summary>
        /// A box position relative to the character's body used to identify collision pairs with nearby objects that could be grabbed
        /// </summary>
        private Box grabPairCollector;

        public Quaternion orientation;
        public Vector3 headOffset;
        private Entity grabbedEntity;
        private BallSocketJoint bsj;
        private NoRotationJoint nrj;
        public bool isFlying;
        public Entity LastTouched
        {
            get { return lastTouched; }
        }
        private Entity lastTouched;
        private Entity touched;
        ...
        private void HighlightEntity()
        {            
            if (_grabbing)
                return;
            touched = null;
            for (int i = 0; i < grabPairCollector.CollisionInformation.Pairs.Count; i++)
            {
                var pair = grabPairCollector.CollisionInformation.Pairs[i];
                EntityCollidable candidate = (pair.BroadPhaseOverlap.EntryA == grabPairCollector.CollisionInformation ? pair.BroadPhaseOverlap.EntryB : pair.BroadPhaseOverlap.EntryA) as EntityCollidable;
                if (candidate == null)
                    continue;
                grabbedEntity = candidate.Entity;
                EntityTag tag = grabbedEntity.Tag as EntityTag;
                if (tag == null)
                {
                    continue;
                }
                ...
                {
                    touched = grabbedEntity;
                    tag.ParentComponent.Mesh.Highlight = true;
                    break;
                }                
            }
            if (lastTouched != null && touched != lastTouched)
            {
                (lastTouched.Tag as EntityTag).ParentComponent.Mesh.Highlight = false;                    
            }
            lastTouched = touched;
        }
        ...
        public void Clip()
        {
            Body.CollisionInformation.CollisionRules.Personal = CollisionRule.Normal;
        }
        ...
        public void ReleaseEntity()
        {
            if (!_grabbing)
                return;
            var eT = grabbedEntity.Tag as EntityTag;
            eT.ParentComponent.Mesh.Highlight = false;
            Space.Remove(bsj);
            Space.Remove(nrj);
            _grabbing = false;
        }
        ...
        void IEndOfTimeStepUpdateable.Update(float dt)
        {
            ...
            grabPairCollector.LinearVelocity = Body.LinearVelocity;
            grabPairCollector.Position = Body.Position + headOffset + (Game1.cam.Transform.Forward * 2.0f);
            grabPairCollector.Orientation = orientation;
        ...
        {
            Body.LinearVelocity = velocity * MaxSpeed;
        }
        ...
        public void Activate()
        {
            if (!IsUpdating)
            {
                ...
                if (Body.Space == null)
                {
                    ...
                    Space.Add(grabPairCollector);
                    Game1.thegame.ModelDrawer.Add(grabPairCollector);
                }
                HasTraction = false;
                IsSupported = false;
        ...
        public void Deactivate()
        {
            if (IsUpdating)
            {
                ...
                if (Body.Space != null)
                {
                    ...
                    Space.Remove(grabPairCollector);
                }
            }
        }
